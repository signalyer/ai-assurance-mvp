...
AI_SYSTEMS: list[AISystem] = [
    ...
    AISystem(
        id="sys-demo-finadvice-001",
        name="Financial Advisor Risk Reviewer",
        description="Advisory agent that reviews a client's portfolio against their stated risk profile and recommends rebalancing actions. Uses deterministic mock data (portfolios, market snapshot, risk profiles) so demo runs are reproducible without live financial APIs. Backing agent: `finadvice` (see agents/_registry.py). All four PII fields (client_name, account_number, tax_id, dob) are tokenised before any model call.",
        business_owner="Diane Reyes, Head of Wealth Advisory",
        technical_owner="SignalLayer Agent Runner team",
        domain="Wealth Management / Advisory",
        ...
        data_classes=[
            DataClass.PII, DataClass.NPI, DataClass.ACCOUNT_NUMBERS,
            DataClass.FINANCIAL_STATEMENTS, DataClass.CUSTOMER_NAMES,
            DataClass.MARKET_DATA,
        ],
        autonomy_level=AutonomyLevel.ADVISORY,
        user_population="Internal — wealth advisors (demo: single operator)",
        customer_impact=CustomerImpact.INDIRECT,
        regulatory_exposure=[
            RegulatoryExposure.GLBA, RegulatoryExposure.CFPB,
        ],
        ...
        rag_sources=[],
        tools=[
            AgentTool(name="lookup_portfolio", description="Fetch a client's current portfolio (mock data)", side_effect=False, authorization_required=False),
            AgentTool(name="lookup_market_snapshot", description="Fetch deterministic market snapshot", side_effect=False, authorization_required=False),
            AgentTool(name="lookup_risk_profile", description="Fetch client's stated risk profile", side_effect=False, authorization_required=False),
        ],
        aws_services=[],
        runtime_status=RuntimeStatus.PILOT,
        release_decision=ReleaseDecision.CONDITIONAL_PILOT,
        inherent_risk=RiskLevel.MEDIUM,
        residual_risk=RiskLevel.LOW,
        use_case="Demonstrate the end-to-end governed agent chain (policy → scrub → guardrails → LLM → eval → memory → audit) on a realistic but mocked financial-advisory workload.",
        human_oversight="Advisory output only — no autonomous trades; advisor reviews every recommendation before acting.",
        data_residency="eastus",
        created_at=NOW - timedelta(days=14),
        updated_at=NOW - timedelta(hours=2),
    ),
    AISystem(
        id="sys-demo-azurearch-001",
        name="Azure Architect (CLI-only Planner)",
        description="Plans Azure deployments via ARM read-only tools and produces ADR-style architecture briefs. CLI-invokable only (its directory name contains a hyphen and isn't a valid Python module path), so it is listed in the Agent Runner picker as DISABLED — present for transparency but not dispatchable from the UI today. Backing agent: `azure-architect` (see agents/_registry.py).",
        business_owner="SignalLayer Platform Engineering",
        technical_owner="SignalLayer Agent Runner team",
        ...
        autonomy_level=AutonomyLevel.ADVISORY,
        user_population="Internal — platform engineers (CLI only)",
        customer_impact=CustomerImpact.NONE,
        regulatory_exposure=[],
        rag_enabled=False,
        rag_sources=[],
        tools=[
            AgentTool(name="arm_list_resources", description="Read-only Azure Resource Manager list calls", side_effect=False, authorization_required=False),
            AgentTool(name="arm_get_resource", description="Read-only Azure Resource Manager get calls", side_effect=False, authorization_required=False),
        ],
        aws_services=[],
        runtime_status=RuntimeStatus.DEV,
        release_decision=ReleaseDecision.NOT_ASSESSED,
        inherent_risk=RiskLevel.LOW,
        residual_risk=RiskLevel.LOW,
        use_case="Demonstrate that agents declaring CLI-only invocation still appear in the AI Systems registry for audit/coverage completeness, even when the Agent Runner cannot dispatch them.",
        human_oversight="All outputs are advisory; no Azure write operations performed by the agent.",
        data_residency="eastus",
