...
**S70b — Evals UI wired to real data + agent Tier-2 memory write:**
- `agents/azure-architect/agent.py` — `call_llm()` post-eval tail now invokes `domain.agent_memory.write_episode()` with the scrubbed prompt, vault_id (reused from the trace_call call), trace_id, eval_summary (small score+passed roll-up; details elided), and outcome derived from eval pass-rate. Skip-aware outcome logic: `skipped` metrics (`score is None`) are excluded from the success/failure decision — any `passed=False` with a real score → "failure", any `passed=True` with a real score → "success", else "review". try/except logs full provenance per `[[bare-except-hides-broken-integrations]]` (`type(exc).__module__.{type(exc).__name__}: {exc}`); failure never blocks the agent. Return dict + `_print_review` footer extended with `episode_id`. Pre-existing bug fixed in the same edit: `anthropic.messages.create()` → `anthropic.messages.stream()` context manager because `TOKEN_BUDGETS["architecture_review"]=4096` exceeds the 2000 streaming threshold per `[[anthropic-max-tokens-streaming-threshold]]`; non-streaming was disconnecting mid-response.
- `api/evaluate.py` — new `GET /api/evals/recent?workload_id=&limit=20` returning the most recent rows from `data/evals.jsonl` (no seed/overlay fallback — empty when no agent has run). Strict Pydantic v2: `RecentEvalMetric` carries the canonical `skipped: bool`, the legacy `passed: bool|None` mark, `score: float|None`. `RecentEvalsResponse` includes a hard-coded `source` string so the SPA can label the panel correctly. Sync `_read_jsonl` wrapped in `asyncio.to_thread`. Filter-before-slice so workload filter doesn't lose rows behind unrelated-recent ones.
- `team-portal/src/pages/evals/RecentLiveRunsPanel.tsx` — new component. Renders one row per recent eval (trace_id, model, workload_id, timestamp + chip row per metric). Skip detection via `skipped` field (not `passed`). Loading guard `loading && !hasData` per `[[never-blank-on-refresh]]`. Empty state: "No live runs yet — run an agent via SDK to populate this." All fetches via `apiGet` (no raw fetch) per `[[raw-fetch-drifts-from-shared-client]]`.
- `team-portal/src/pages/evals/EvalsPage.tsx` — mount `<RecentLiveRunsPanel />` above the KPI row. Seed-overlay system cards below are unchanged (intentional — the two surfaces have different provenance and shouldn't be merged).
- `team-portal/src/pages/evals/types.ts` — `RecentEvalMetric` / `RecentEvalRow` / `RecentEvalsResponse` interfaces mirror the engine's strict Pydantic v2 shape.
- `tests/test_api_evals_recent.py` — 3 tests using tmp_path-isolated JSONL: empty file → honest empty response; three rows → most-recent-first ordering + skipped metric round-trips correctly; workload_id filter + limit clamp work together. The path override is on `api.evaluate._EVALS_JSONL_PATH` directly (not env var) — `evaluator.py` captures `DATA_ROOT` into its own constant, so env-var twiddling would not affect the test target.
- Local-only insights: `agents/azure-architect/.env` needs `DATABASE_URL=postgresql://...psql-aigovern-dev...` for episode persistence to land in real Postgres, but the dev DB firewall blocks the developer IP — locally `write_episode` always returns "" and logs the connection timeout. Persistence happens when the engine itself runs agents (S71+ SDK decorator for `episode-write` will fix the architecture). Local Python also needs `pip install sqlalchemy>=2.0.0 psycopg2-binary>=2.9.0` because the agent imports `domain.agent_memory` which transitively requires them — those are engine-side deps that the local agent inherits today.

Live tips after S70b: engine deployed cleanly from `deploy/app.zip` to `app-aigovern-dev` (BUILD_SHA baked from local tip); team-portal SPA `index-Cs7pTbyG.js` on `portal.aigovern.sandboxhub.co` (S69 was `index-CaTHiUwg.js`), 3 string markers grep-verified on the live bundle ("Recent live runs", "/evals/recent", "No live runs yet"); ciso-console SPA `index-bVhd18Tk.js` (still unchanged from S66).

**S71 Block A — SDK `write_episode` over signed HTTP (closes the S70b architectural gap):**
- `api/sdk_episodes.py` — new router `POST /api/sdk/episodes` under the `/api/sdk/*` HMAC prefix (auth inherited from `HMACAuthMiddleware`, NOT `SessionAuthMiddleware` — agents have no cookies). Strict Pydantic v2: `EpisodeWriteRequest` (workload_id min_length 1, prompt, response, outcome, metadata, ttl_seconds≥0, `extra="forbid"`) → `EpisodeWriteResponse` (`episode_id`, `extra="forbid"`). Delegates to `domain.agent_memory.write_episode()` via `asyncio.to_thread` (sync sqlalchemy under the hood), so the engine's configured memory backend (postgres in prod, jsonl in dev) decides where the row lands — endpoint stays backend-agnostic. Error mapping: invalid outcome → 400 pre-write; `ValueError` from backend (SCRUBBER_ENABLED missing vault_id) → 400; any other backend exception → opaque 500 `"episode_write_failed"` per `[[bare-except-hides-broken-integrations]]` (log full provenance, never leak the timeout text). Operation id `sdk_episodes_write`, status_code 201.
- `sdk/signallayer/__init__.py` — new top-level `write_episode(workload_id, prompt, response, outcome, metadata=None, ttl_seconds=None) -> Result`. Thin wrapper that calls `get_client().post("/api/sdk/episodes", json_body=...)`. Returns `Ok[episode_id]` on success or `Err` on auth/validation/network/DB failure — same typed-discrimination contract S29 established for other SDK calls (no exceptions for expected modes). `ttl_seconds` omitted from body when `None` so the engine default applies. Added to `__all__`.
- `agents/azure-architect/agent.py` — `call_llm()` migrated off the direct `from domain.agent_memory import write_episode` import; now `from signallayer import Err, write_episode`. Customer agent box no longer needs `sqlalchemy` / `psycopg2-binary` (transitively pulled by `domain.agent_memory`). `Err` branch logs `status_code` + `type(error).__name__` + message via stderr; the outer `except` catches the unexpected non-SDK path (e.g. `signallayer.init()` not called). episode_id stays `""` on either failure path — agent never blocks on memory.
- Same edit folded in the S70b carryover refactor: outcome logic now keys off `payload.skipped` (canonical signal per `evaluator.py:57`) instead of `score is None` (correlated 1:1 but indirect). `eval_summary` now carries `skipped: bool` per metric in the metadata payload — useful for the future episode browser without rejoining to evals.jsonl.
- `dashboard.py` — `from api.sdk_episodes import router as sdk_episodes_router` + `app.include_router(sdk_episodes_router)` slotted between `sdk_runtime_router` and `frameworks_router` (other sdk_* routers are clustered there).
- `tests/test_api_sdk_episodes.py` — 5 tests via `FastAPI` + `HMACAuthMiddleware` + `TestClient`. Patches `api.sdk_episodes.write_episode` so Postgres is never touched. Cases: signed 201 + episode_id round-trip; unsigned → 401 (middleware blocks before route runs); invalid outcome → 400 pre-write; backend `ValueError` (vault_id missing) → 400; backend `RuntimeError` → opaque 500 (`detail == "episode_write_failed"`, never leaks the inner message). Reuses the signing helper byte-for-byte from `test_hmac_auth.py`.
- `tests/test_sdk_write_episode.py` — 4 unit tests with `SignalLayerClient.post` mocked. Confirms happy path returns `Ok[str]` with the episode_id; `ttl_seconds` only forwarded when provided; `Err` (e.g. `AuthError 401`) passes through unchanged; missing `episode_id` in a 200 body returns `Ok("")` rather than crashing.

Live tips after S71 Block A: engine ships the new HMAC-guarded route. Customer agent venv no longer requires `sqlalchemy` + `psycopg2-binary`. Local `python agent.py --review` against `SL_API_BASE_URL=https://aigovern.sandboxhub.co` lands episodes in the deployed Postgres without touching the developer-IP firewall — closes the S70b "only verifiable from inside App Service" gap.

**Open issues at end of S71 Block A:**
- Block B (LLM action surface rollout — `ask` / `summarize-finding` / `summarize-evidence` / `draft-report` streaming on both SPAs) deferred from this session. Plan still in `docs/plans/SESSION-71-*.md`.
- Anthropic-pin still in place on `FailedGateRow.onExplain` per S69; drop in S71b when Bedrock streaming adapter ships.
- DeepEval intermittent `APIConnectionError` on `answer_relevancy` when 3 evals run within ~1 min (rate-limit on the metric model). Not blocking but noisy in the Evals panel.
- Dev Postgres admin password surfaced in the chat transcript via `az` lookup. Rotating it post-S70b is reasonable hygiene; not a leak per se (already in App Service settings + repo committers' shells).

**S72 — LLM action surface rollout (trimmed scope: 4 on team-portal + summarize-finding on ciso-console):**
- `domain/assurance_providers.py` — `_build_prompt()` extended with branches for `FINDINGS_SUMMARIZATION`, `EVIDENCE_SUMMARIZATION`, `EXECUTIVE_REPORT_GENERATION`, `SYSTEM_QA` (S69 shipped only RELEASE_DECISION_NARRATIVE). Each prompt is grounded against the `simulate_response()` framing the operator already sees — that text is the calibration target, so drawer rendering needs no per-use-case work. `_MAX_TOKENS_BY_USE_CASE` table pre-existed from S69; values stand. All four go through `stream_anthropic_response` and therefore through `client.messages.stream()` per `[[anthropic-max-tokens-streaming-threshold]]`.
- `api/assurance_model.py` — `ask`, `summarize-finding`, `summarize-evidence`, `draft-report` flipped from `_dispatch` (sync, sim-only `AskResponseOut`) to `_dispatch_streaming` returning `EventSourceResponse`. Dropped `response_model=AskResponseOut` on all four (OpenAPI can't represent SSE; client parses the terminal `done` event against the TS interface). Each handler now takes `request: Request` for client-disconnect detection in the live path. `_stream_live_release_narrative` is misnamed (it's actually use-case agnostic) but renaming is churn — left.
- `tests/test_api_assurance_model.py` — `_S72_ROUTES` parametrize matrix (4 routes × 3 scenarios = 12 new tests) extending the S69 trio. Covers (a) sim-when-flag-off, (b) live-streams-with-mocked-Anthropic (delta count + token + streaming_complete asserted; dispatcher's use_case forwarding asserted via `expected_use_case` closure-captured before the inner `fake_stream` shadows it), (c) sim-fallback-when-no-creds. Pre-existing S69 trio still green. 15/15 pass. The `"Coverage"` → `"Evidence summary"` author-time miss in the evidence test marker is the same shape as `[[smoke-scripts-must-run-live]]` (sim text was the legacy framing, not my new prompt).
- `team-portal/src/pages/ai-systems/AiSystemDrawer.tsx` — inline buttons (no menu): per-row `Summarize` on each open finding row, `Summarize evidence` on the Evidence section header (disabled when rows=0), `Ask AI` + `Draft Report` in the drawer action row. `Ask AI` uses `window.prompt()` for the question — minimal UX, S73 may refactor to drawer-resident input. Each button assembles use-case-specific payload context from the drawer's in-memory `AiSystemDetail` + `evidenceRows.value` (no extra fetch). All four pin `preferred_provider: 'anthropic-prod'` per `[[FailedGateRow.onExplain pattern]]` until Bedrock streaming adapter ships.
- `team-portal/src/pages/ai-systems/AiSystemDrawer.tsx` — `EvidenceSection` now takes `systemId` prop (was unused before; needed for the new `Summarize evidence` button context). `summarizeOpenFindings()` + `summarizeEvidenceSections()` helpers collapse the drawer's existing state into compact prompt context strings.
- `ciso-console/package.json` + `package-lock.json` — added `@microsoft/fetch-event-source@^2.0.1` (first SSE dep on this SPA).
- `ciso-console/src/shared/api/client.ts` — `apiSse()` wrapper ported verbatim from team-portal S69. Preserves `credentials:'include'` + `X-Data-Mode` header per `[[raw-fetch-drifts-from-shared-client]]`; `openWhenHidden:true` so backgrounded tabs don't kill the stream.
- `ciso-console/src/shared/types/assurance.ts` — new file. `AskRequest` + `AskResponseOut` + `GovernanceMetadata` + `PolicyDecisionOut` types mirrored verbatim from team-portal. Comment marks them as "keep in sync."
- `ciso-console/src/shared/components/AiSummaryDrawer.tsx` — new file. Drawer ported verbatim from team-portal S69 (signal-driven open state, streaming state machine, abort-on-close, copy-markdown button, routing dl with token + cost rows, partial-stream banner). Uses `mono` (ciso-console class) instead of team-portal's `font-mono` because the two SPAs have different base CSS class names — caught at build time.
- `ciso-console/src/shared/components/Shell.tsx` — `<AiSummaryDrawer />` mounted as a sibling to `<main>` so any page can call `openAiSummary({...})`.
- `ciso-console/src/shared/styles/base.css` — appended `@keyframes aiCursorBlink` for the streaming cursor (the cursor `<span>` references the animation via inline style; tree-shakes the name into the JS bundle but the keyframe declaration itself lives in CSS).
- `ciso-console/src/pages/findings/FindingsInboxPage.tsx` — Findings drawer body gets a `Summarize with AI` button (deliberate label differentiation vs team-portal's terse `Summarize` — CISO context wants the AI affordance visible). `openSummarizeFinding(f)` helper packs finding context + Anthropic pin. Note: passes `f.priority` (P0-P3) into the prompt's `severity` slot — semantic mismatch; the prompt treats it as a label string so it surfaces correctly but the Finding model should expose `severity` properly in a future pass.
- Engine deployed via `.github/workflows/deploy.yml` on push to `main` — same workflow that's been running since S57. Push `c05a566` carried S70b + S71 Block A + S72 (the prior 6 commits had been deployed via direct zip but never pushed to origin; this push fast-forwarded origin/main from `e532ff7` to `c05a566`).
- Both SPAs deployed via manual `swa deploy ./dist --deployment-token <tok> --env production` per `[[spa-deploy-is-manual-swa]]`. Tokens fetched fresh via `az staticwebapp secrets list` — they don't rotate.
- Post-deploy verification: bundle hashes on the live origins match the build output (`index-D6YPZap2.js` on `portal.aigovern.sandboxhub.co`; `index-BG5ryvcJ.js` on `gov.aigovern.sandboxhub.co`). String-grep on both bundles confirms: 7 expected markers on team-portal (4 endpoint paths + `Ask AI` + `Draft Report` + `Summarize evidence`), 3 on ciso-console (`Summarize with AI` + `summarize-finding` endpoint + `aiCursorBlink` keyframe name). All 5 streaming routes return clean 401 on unauthenticated probe — not 404 — confirming engine deploy picked up new dispatcher per `[[auth-shadows-404]]` discriminator.

Live tips after S72: engine commit `c05a566` deployed to `app-aigovern-dev`; team-portal SPA `index-D6YPZap2.js` on `portal.aigovern.sandboxhub.co`; ciso-console SPA `index-BG5ryvcJ.js` on `gov.aigovern.sandboxhub.co` (was unchanged from S70b's `index-bVhd18Tk.js` for the first time since S66). The full Python suite reports 350 passed / 8 skipped / 2 failed; the 2 failures (`test_agents_unit::test_08_list_agents_returns_empty_list_no_engine` flaky in full-run only, passes in isolation; `test_session10_hardening::test_rtf_index_sidecar_used` complaining about "unsigned (legacy entry)" sidecar entries) are pre-existing and outside the S72 blast radius — carried to S73 for triage.

**Open issues at end of S72:**
- ciso-console only has `summarize-finding` from the 4 new actions; `ask` / `summarize-evidence` / `draft-report` deferred to S73 per locked Q1 trimmed-scope decision.
- Anthropic-pin still in place on all 5 streaming buttons (S69's `FailedGateRow.onExplain` + S72's 4 new). Drop in S73 when Bedrock streaming adapter lands.
- S71 carryover untouched: dev Postgres admin password rotation, `AGENTS.md` / `team-portal/cookies.txt` triage, agent_memory integration test deferral.
- 2 pre-existing test failures noted above need triage in S73.
- `Ask AI` button uses `window.prompt()` — functional but janky in a drawer context. S73 may move to a drawer-resident input.
- `EvidenceSection` summarize-context is computed client-side from the in-memory rows (`evidence_sections` is a CSV of evidence types). Operators with 20+ evidence rows will see a long sections string in the prompt; if this hurts response quality, fall back to server-side evidence summary in S73.
- AiSystemDrawer header gets crowded with `Ask AI` + `Draft Report` next to the existing 4 buttons (Edit / Revisions / Frameworks / Bound Agents). A 7-button row is at the ceiling — adding a 7th action means we refactor to an "AI Actions" dropdown menu per the deferred S72 Q3 option.

**S73 — ciso-console LLM action port + AiActionsMenu refactor:**
- `team-portal/src/shared/components/AiActionsMenu.tsx` — new shared dropdown component. Takes `items: AiActionItem[]` (`{key, label, onClick, disabled?}`) and an optional `label` (defaults to "AI Actions"). Click-outside + Escape + scroll-close. Uses `--bg-elevated` / `--bg-card-hover` / `--border` / `--text-primary` per `[[css-var-fallbacks-break-theming]]` — first draft tried nonexistent `--surface-1/2` and shipped invisible; caught at SPA-vars audit before deploy. **Positioning took 3 iterations to land**: v1 `position: absolute` was clipped by drawer scroll-region overflow; v2 `position: fixed` with `getBoundingClientRect` coords was re-anchored by `.drawer { transform: translateX(...) }` (transformed ancestors become the containing block for fixed descendants per CSS spec) — menu rendered offscreen relative to the translated drawer; v3 `createPortal(node, document.body)` + `position: fixed` escapes both clip and transform. zIndex 1000 so it sits above table chrome but below the AiSummaryDrawer (z=300+). See `[[dropdown-in-transformed-ancestor-needs-portal]]`.
- `team-portal/src/pages/ai-systems/AiSystemDrawer.tsx` — drawer action row's `Ask AI` + `Draft Report` inline buttons collapsed into one `<AiActionsMenu>` instance. Operational buttons (Edit / Revisions / Frameworks / Bound Agents / Run Assessment) stay inline by design — menu earns its keep only at 2+ AI actions per surface. Drops the drawer row from 7 to 6 controls.
- `ciso-console/src/shared/components/AiActionsMenu.tsx` — verbatim copy of team-portal's. Duplicate-per-SPA pattern continued (S72 precedent) — no workspace package until 3+ shared components justify it.
- `ciso-console/src/pages/portfolio/PortfolioPage.tsx` — Top-10 systems table row's Action cell now carries `<AiActionsMenu>` (Ask AI… + Draft Report) next to the existing Findings link. Inline helpers `openAskAboutSystem`, `openDraftReport`, `submitAskPrompt`, `summarizeFindingCounts`, and the `AskAiPromptModal` component ported from team-portal/AiSystemDrawer but adapted for `AiSystemSummary` shape (counts, not full `findings[]`). `evidence_summary` slot receives a placeholder ("Evidence detail not loaded at portfolio surface…") — degraded grounding vs team-portal drawer (which has live `evidenceRows`), but the S72b empty-evidence path proves the prompt handles missing evidence gracefully.
- `ciso-console/src/pages/evidence/EvidencePage.tsx` — `Summarize this view` button added in `EvidenceCard` header next to filter selects. Inline button (single action — no menu). Binds to the existing `filteredRows` signal so the prompt sees exactly what the operator sees. Scope label is `filterSystem.value` ("ALL" → portfolio-wide, else per-system); `ai_system_id` is nullable in `AskRequest` so portfolio-wide passes `null`. New helper `summarizeEvidenceSections()` ported verbatim from team-portal; `openSummarizeFilteredEvidence()` is the wire-up.
- All 3 new ciso-console actions pin `preferred_provider: 'anthropic-prod'` per S72 — Bedrock streaming adapter still deferred (S74 Secondary block).
- Both SPAs deployed via manual `swa deploy ./dist --deployment-token <tok> --env production`. Bundle hashes: team-portal `index-DHlFuQxe.js` (was `JHuCPXCT`), ciso-console `index-BtyZTC8Z.js` (was `B-EVFGv0`). String-grep against live origins confirms: team-portal bundle ships `AI Actions` (menu); ciso-console ships `AI Actions`, `Ask AI`, `Draft Report`, `Summarize this view` (3 ported actions + menu). Minified helper names don't survive prod build — user-facing strings are the reliable signal.

**Open issues at end of S73:**
- Pre-existing `--surface-2` reference in `ciso-console/src/pages/evidence/EvidencePage.tsx:351` (expanded-row background) is dead — same shape as the AiActionsMenu bug caught here. Not introduced by S73 but discovered during the SPA-vars audit. S74 triage.
- Anthropic-pin still in place on all 8 streaming buttons (S69's `FailedGateRow.onExplain` + S72's 4 on team-portal + S73's 3 on ciso-console). Bedrock streaming adapter is S74 Secondary block.
- S71+S72 carryover still untouched: dev Postgres admin password rotation, `AGENTS.md` / `team-portal/cookies.txt` triage, agent_memory integration test deferral, 2 pre-existing test failures (`test_agents_unit::test_08_…` order-dependent, `test_session10_hardening::test_rtf_index_sidecar_used` unsigned legacy entry).
- ciso-console's PortfolioPage Draft Report has degraded evidence grounding vs team-portal (no `evidenceRows` at the portfolio surface). Acceptable for V1; an enhancement would prefetch per-row evidence counts in the portfolio listing.
- `_stream_live_release_narrative` still misnamed in `domain/assurance_providers.py` (use-case agnostic since S72). Rename deferred to S74 when Bedrock branch lands and the file gets touched anyway.

**S74 — Carryover cleanup (Primary block deferred to S75; user has no AWS subscription yet):**
- S74 Primary (Bedrock streaming adapter) was blocked at the verify-AWS-credentials step. User confirmed Azure-only — AWS subscription provisioning is now a prerequisite. Pivoted entire session to Secondary block.
- `tests/test_agents_unit.py` — autouse fixture clears `domain.agents._inmem_agents` before/after each test. test_08 fails because test_04's no-engine `create_agent` populates the in-memory fallback dict that never resets. Plan diagnosed as full-run pollution; reality is it fails as soon as test_04 runs before test_08, even within `test_agents_unit.py` alone.
- `tests/test_session10_hardening.py::test_rtf_index_sidecar_used` — fixture now writes a *signed* sidecar entry via `rtf._compute_sidecar_sig` + a monkeypatched `SL_HMAC_SECRET`. S11 hardening rejects unsigned entries; pre-hardening fixture shape was the bug.
- `tests/test_agent_memory_chain.py` — NEW. S70b deferral closed. Two tests: (a) stacks `@policy_gate` + `@scrub_pii` on a fake async fn, spies on `domain.policy_engine.evaluate` to confirm chain order (policy sees RAW prompt, body sees SCRUBBED prompt), then asserts the inline `write_episode` tail receives the scrubbed prompt + vault_id — the actual `CLAUDE.md` security invariant ("Langfuse gets scrubbed_prompt — never raw_prompt"); (b) fail-closed test proving the wrapped fn is NOT called if scrubber errors. Hermetic via `patch("scrubber.tokenise_payload", _fake_tokenise)` because the provider-cache `@lru_cache` singleton survives across tests — earlier tests pin a noop backend and pollute the real scrubber for this one. Initial draft used real Presidio and passed in isolation but failed in full-run; lesson: provider-cache singletons compound test pollution, mock at the lazy-import seam.
- `api/assurance_model.py` — `_stream_live_release_narrative` renamed to `_stream_live_assurance_response`. Misnamed since S72 (use-case agnostic). Two call sites, clean rename, import-check passes.
- `ciso-console/src/pages/evidence/EvidencePage.tsx:398` — `var(--surface-2)` replaced with `var(--bg-card-hover)` for the expanded-row background. Per `[[css-var-fallbacks-break-theming]]` — `--surface-2` is undefined in `ciso-console/src/shared/styles/base.css` and would render with no inline-style fallback (transparent expanded row). Bundle `index-DsSxQhUZ.js` deployed to `gov.aigovern.sandboxhub.co` via `swa deploy`; bundle-hash grep + HTTP 200 verified live.
- `AGENTS.md` — committed (was untracked since S69). Near-mirror of CLAUDE.md with one extra compound rule (2026-05-27 `spec_from_file_location` lesson on Pydantic v2 forward refs). Kept as separate file for OpenAI Codex / agent-tool compatibility convention.
- `.gitignore` — added `cookies.txt` + `**/cookies.txt`. Removed `team-portal/cookies.txt` (Azure ARRAffinity test debris from S69 curl probes).

**Open issues at end of S74:**
- **AWS subscription provisioning** — gating S75 Primary (Bedrock streaming adapter). When ready, user provides `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY` + `AWS_REGION` (default `us-east-1`) plus confirmation that Claude 3.5 Sonnet v2 model access is *Granted* in the Bedrock console (separate click-through). IAM user should have least-privilege `bedrock:InvokeModelWithResponseStream` + `bedrock:InvokeModel` only.
- Anthropic-pin still in place on all 8 streaming buttons (1 in team-portal `FailedGateRow`, 4 in team-portal `AiSystemDrawer`, 1 in ciso-console `FindingsInboxPage`, 2 in ciso-console `PortfolioPage`, 1 in ciso-console `EvidencePage`). Drops in S75 after Bedrock adapter is proven via `/explain-release` live smoke.
- dev Postgres admin password rotation deferred — needs a password from the user. ~15 min S75 prep step.
- ciso-console's `PortfolioPage` Draft Report still has degraded evidence grounding vs team-portal (no `evidenceRows` at the portfolio surface). Optional S75 enhancement.
...
**S79 — Dual-path dispatcher + tool-using agent governance (S79.3 + S79.4 + S79.5 of 8 planned):**
- `domain/assurance_providers.py::_LOCAL_SIMULATED` — new internal-path provider (`provider_id="local-simulated"`, `ProviderType.LOCAL`, status `CONNECTED`). Allow-list covers SYSTEM_QA + FINDINGS_SUMMARIZATION + EVIDENCE_SUMMARIZATION + RELEASE_DECISION_NARRATIVE + EXECUTIVE_REPORT_GENERATION + POLICY_CONTROL_QA + HALLUCINATION_GRADING + GROUNDEDNESS_SCORING + PRODUCTION_INFERENCE. All data classes allowed (zero-egress). `have_real_credentials` returns `True` for this provider only (still `False` for the placeholder `local-vpc`). Distinct from `local-vpc` which remains the customer-VPC-deployment placeholder.
- `domain/assurance_providers.py::stream_local_response` — async generator wrapping `simulate_response`. Yields whitespace-chunked deltas with `asyncio.sleep(0)` between yields (so SSE pipe flushes per chunk, visually identical to a real LLM stream in the drawer). Terminal `("done", …)` event contract matches Anthropic + Bedrock exactly: `input_tokens=0`, `output_tokens=len/4`, `cost_estimate_usd=0.0`. Same governance chain runs (policy gate → sanitize → audit → T2 episode write); only the upstream byte source differs.
- `api/assurance_model.py::_stream_live_assurance_response` gained a third branch: `provider_type == LOCAL → stream_local_response`. All three streaming providers (Anthropic, Bedrock, Local) now share the dispatcher's single audit + S78 episode-write tail.
- `agents/azure-architect/agent.py::_run_plan` — closed the tool-using-agent governance gap memorialized in `[[ui-promise-audit-owed]]`. Decorator chain `@policy_gate(action="llm_call") → @scrub_pii(scope="azure-architect-plan") → @guardrails()` now applied. First positional arg renamed `operator_request` → `prompt` (internal `operator_request = prompt` alias keeps the message-build/synthesis-log lines unchanged) so `scrub_pii._extract_prompt` finds it. `vault_id` + `workload_id` kwargs injected by decorators; fallback synthesised from prompt hash when `SCRUBBER_ENABLED=false` (same pattern as `call_llm`). SDK `write_episode` call added at synthesis time — outcome derived from terminal stop_reason: `end_turn → success`, `turn_cap_reached → failure`, anything else → `review`. Per-turn re-scrub of tool-result blocks is explicit S85 (Tier-3 health) scope, not in this session.
- `agents/azure-architect/agent.py` `__main__` updated: `_run_plan(prompt=request, subscription_id=subscription, ...)` (kwarg-style — `scrub_pii` prefers `prompt` kwarg over positional).
- **`write_episode` caller audit (S79.5)** completed. All 6 prod callers either run inside the chain (`api/assurance_model.py` x2, `agents/azure-architect/agent.py` call_llm + _run_plan), are pre-scrubbed at the surrounding boundary (`api/demo_run.py`), or are the documented external SDK entry point (`api/sdk_episodes.py`). One soft flag: `api/memory.py:188` (`POST /api/memory/episodes`) is an internal endpoint that relies on caller-supplied `vault_id`; `domain.agent_memory.write_episode` fail-closes when `SCRUBBER_ENABLED=true` and metadata lacks `vault_id`, so it's defense-in-depth — but the public-API entry deserves explicit auth/role verification in S84 (RBAC SPA + UI-promise audit).
- **Handoff-vs-code drift caught:** the S79 plan said `agents/azure-architect/agent.py` *bypasses* policy/scrub/guardrails. That was wrong for `call_llm` (whose SDK decorator stack is the canonical chain tested in `tests/test_agent_memory_chain.py:47`). The real gap was `_run_plan` (the `--plan` tool-use loop). Re-scoped to that surface; refactoring `call_llm` would have broken the load-bearing chain test per `[[grep-all-consumers-before-contract-flip]]`.

**S79 deferred to S80:**
- S79.1/S79.2 (live intake + portfolio + Agent Library verify with `sys-demo-finadvice-001` + `sys-demo-compliance-001`) — defer to next session; sys-demo-compliance-001 will be tagged with PII/NPI so the routing engine auto-prefers `local-simulated` over Anthropic (zero-code-change narrative per `[[grep-all-consumers-before-contract-flip]]`).
- S79.6 (eval suite UI → real episodes) — S70b carryover, untouched.
- S79.7 (eval-failure → finding auto-create glue: `<0.6 → P2`, `<0.4 → P1`) — `domain/findings_workflow.py`, untouched.

**Test status at end of S79 (code-shape):** modules import cleanly (`domain.assurance_providers`, `api.assurance_model`, `agents/azure-architect/agent.py` parses). No full test suite run this session — deferred to deploy + live verify in S80 per `[[pytest-py314-capture-teardown]]` (use `-s` flag).

**S80 — Agent Runner backend (chain-event SSE dispatcher + finadvice agent + registry):**
- `agents/__init__.py` — namespace marker so `agents._registry` is importable.
- `agents/_registry.py` — `AgentSpec` dataclass + `REGISTRY` tuple + `get_agent / list_registered_agents / load_agent_entrypoint / load_agent_inner`. Two registered agents: `finadvice` (runner-invocable) and `azure-architect` (`cli_only=True`, picker-visible but un-dispatchable because the directory name's hyphen breaks `importlib.import_module`). `AgentSpec.inner_entrypoint` field added so the chain-event dispatcher can resolve an UNDECORATED inner — separate from `entrypoint` which the CLI/direct callers use. `load_agent_inner` falls back to `entrypoint` when `inner_entrypoint` is None.
- `agents/finadvice/{__init__,agent,prompts}.py` + `mocks/{portfolios,market,profiles}.json` + `.env.example` — first runner-invocable agent. Mirrors `agents/azure-architect/agent.py` shape: SDK decorator chain (`@policy_gate → @scrub_pii → @guardrails`), 5-turn Anthropic tool-use loop with `messages.stream()` (per `[[anthropic-max-tokens-streaming-threshold]]`), `write_episode` at synthesis. Three tools (`get_client_portfolio` / `get_market_snapshot` / `get_client_risk_profile`) read deterministic JSON fixtures with realistic PII fields on three clients. Calibration against cln-001 (Sonnet 4.6, 3 turns, 32s, stop=end_turn) produced a real fiduciary-grade review naming top 3 positions (NVDA/MSFT/AAPL), citing target-tech-band constraint (40–50%) + NVDA 30d vol (42%), and emitting 3 ranked rebalance actions — all three calibration asserts pass first-pass.
- **Wrapper/inner split in finadvice (S80 design lock):** `_run_review_inner` is the undecorated body taking an optional `event_sink: Callable` for per-token `llm.delta` emission. `run_review = signallayer.policy_gate(...)(signallayer.scrub_pii(...)(signallayer.guardrails()(_run_review_inner)))` is the decorated public surface CLI + direct callers use. Lets the chain-event dispatcher run policy/scrub/guard manually with per-step timing instead of treating the decorator stack as a black box. Same pattern applies to future runner-invocable agents; azure-architect deferred per S80-entry decision.
- `domain/agent_runner.py::stream_agent_run_with_chain_events` — async generator emitting the 10-event protocol locked at S80 (LBD-1 in `docs/plans/SESSION-80-agent-runner.md`): `chain.start, policy_gate, scrub_pii, guardrails, llm.delta*, llm.done, evaluate, memory, audit, chain.done` plus `chain.error` on any uncaught. Every event carries `event` / `run_id` / `elapsed_ms`. Calls `domain.policy_engine.evaluate` + `scrubber.tokenise_payload` + `middleware.guardrails.GuardrailsMiddleware.check_input` inline with per-step timing, then dispatches to the agent's `_run_review_inner` via a `asyncio.Queue`-backed sink so deltas interleave with the agent's progress rather than dump at the end. Generator NEVER raises — every error path emits `chain.error` followed by `chain.done` so consumers can rely on a terminal event.
- `api/agent_runner.py` — `GET /api/agent-runner/agents` (returns registry catalog including `cli_only` agents so the SPA renders them disabled) + `POST /api/agent-runner/run` (SSE wrapping the dispatcher; user pulled from signed session cookie via `_read_cookie`). Both routes require any role in `_RUNNER_ROLES = (operator, architect, ciso, auditor, admin)`. `RunRequest` Pydantic model with `extra="forbid"`.
- `dashboard.py` — eager-imports both `api.agent_runner` (router) AND `agents._registry` (so the registry's bound agent modules load at startup) per `[[lazy-imports-skip-module-load-bootstrap]]`. Both routes verified mounted at `/api/agent-runner/agents` and `/api/agent-runner/run`.
- `docs/agent-runner-sse-protocol.md` — formal schema for the 10 event types so future agents extending the protocol have a single source of truth (per `[[grep-all-consumers-before-contract-flip]]`).

**S80 architectural findings (worth capturing):**
- **Policy DENY on raw operator PII fires BEFORE `@scrub_pii` runs.** `@policy_gate` is the outermost decorator (per chain order), and `domain/policy_engine.py:373` enforces `pii_no_raw_to_external` — denies any operator prompt containing SSN / email / credit-card patterns. Calibration ran with PII-free operator prompts and the redaction story for the chain-event protocol is honest at S80: operator prompts that pass policy never have raw PII to scrub. The richer PII-redaction demo (tool-result PII flowing back from `get_client_portfolio`) requires per-turn tool-result re-scrubbing — explicit S85 (Tier-3 health) scope per the S79 caveat.
- **`functools.wraps` copies `__name__` from inner to wrapper**, so name-based identity checks `func.__name__ == "_run_review_inner"` is unreliable — both the decorated wrapper and the undecorated inner report the same `__name__`. `inner is decorated` (object identity) is the only honest check; `load_agent_inner` returns the right object even when registry name resolution looks ambiguous.
- **Anthropic `text_stream` must be consumed BEFORE `get_final_message()`.** When the dispatcher passes an `event_sink`, `_run_review_inner` iterates `stream.text_stream` to emit per-chunk `llm.delta` events, then `get_final_message()` returns the assembled message. When `event_sink is None` (CLI path) the iteration is skipped so behaviour is byte-identical to the pre-S80 path.

**Tests at end of S80:** `tests/test_agent_runner_chain_events.py` — 9/9 pass in 0.22s with `-s`. Covers event order, uniform key contract, terminal event, run_id consistency, delta-count fidelity, unknown agent_id graceful failure, inner-exception → chain.error, scrub_pii payload shape, and `demo_mode` raw_preview gating.

**S80 deferred / next session (S81 — SPA):**
- Team-portal `/agent-runner` page consuming `POST /api/agent-runner/run` via `EventSource`. Chain ticker UI rendering all 10 event types with elapsed_ms badges. Picker calling `GET /agents`.
- Side-by-side dual-path columns (S82 — needs second `system_id` routed to `local-simulated` per the S79 dispatcher) and PII redaction preview (S82 — gated on `DEMO_MODE=true` in App Service settings).
- `audit` event Langfuse + AppInsights deep links (S83 — env-driven URL builders).
- Per-turn tool-result re-scrub (S85 — Tier-3 health; the structural place to land the richer redaction demo).
