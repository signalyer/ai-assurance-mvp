...
**Open issues at end of S70b:**
- Postgres firewall blocks local IP from writing episodes — architecturally correct, but means the demo path "agent → episode row visible in psql" is only verifiable from inside the App Service. S71's SDK decorator for `episode_write` (POST via signed HMAC, same shape as `policy_gate`) closes this properly.
