...
| ~~Parent-domain cookie wired via `SESSION_COOKIE_DOMAIN` env var (Session 24) is unset on V1 today~~ ✓ **ACTIVATED Session 25 (2026-05-24).** `SESSION_COOKIE_DOMAIN=.aigovern.sandboxhub.co` set on `app-aigovern-dev` via `az webapp config appsettings set`; app restarted clean; both `aigovern.sandboxhub.co` + `api.aigovern.sandboxhub.co` serve `/api/health` 200. Pre-existing custom domain bind on `api.aigovern.sandboxhub.co` (SNI SSL active, thumbprint `9287FDA1...`) discovered during Session 25 Track C execution — A4 was effectively closed out-of-band before Session 25. **Remaining manual verification (one real login required):** open DevTools → Application → Cookies on `https://aigovern.sandboxhub.co` after login, confirm the `session` cookie shows `Domain=.aigovern.sandboxhub.co` (not host-only) and that logout removes it. |
